...
export interface DoctorFinding { severity: FindingSeverity; title: string; detail: string; fix?: string; }
export interface ProjectReport {
    root: string; pythonFiles: number; framework: Framework; hasVenv: boolean;
    hasRequirements: boolean; hasPyproject: boolean; hasGitignore: boolean;
    envKeys: string[]; imports: string[]; missingLikelyPackages: string[]; findings: DoctorFinding[];
}

// Only explicit mappings are auto-installed. Project-local modules are never sent to PyPI.
const IMPORT_TO_PACKAGE: Record<string, string> = {
    ...
    psycopg2: 'psycopg2-binary', requests: 'requests', httpx: 'httpx', aiohttp: 'aiohttp',
    sqlalchemy: 'SQLAlchemy', pymongo: 'pymongo', redis: 'redis', openai: 'openai', anthropic: 'anthropic',
    transformers: 'transformers', torch: 'torch', pandas: 'pandas', numpy: 'numpy', matplotlib: 'matplotlib',
    plotly: 'plotly', pytest: 'pytest', rich: 'rich', psutil: 'psutil', PySide6: 'PySide6',
    PyQt6: 'PyQt6', PyQt5: 'PyQt5', lxml: 'lxml', magic: 'python-magic', bcrypt: 'bcrypt',
    cryptography: 'cryptography', jinja2: 'Jinja2', werkzeug: 'Werkzeug', uvicorn: 'uvicorn',
    gunicorn: 'gunicorn', fastapi: 'fastapi', flask: 'Flask', django: 'Django', streamlit: 'streamlit',
...
function collectPythonFiles(root: string, maxFiles = 400): string[] {
    ...
    const walk = (dir: string) => {
        ...
        for (const entry of entries) {
            if (result.length >= maxFiles || ignored.has(entry.name)) break;
            const full = path.join(dir, entry.name);
            if (entry.isDirectory()) walk(full); else if (entry.isFile() && entry.name.endsWith('.py')) result.push(full);
        }
    };
...
function detectImports(files: string[]): string[] {
    ...
    for (const file of files) {
        const text = readText(file); let match: RegExpExecArray | null;
...
function declaredPackages(root: string): Set<string> {
    ...
    if (fs.existsSync(requirements)) for (const line of readText(requirements).split(/\r?\n/)) {
        const value = line.trim().replace(/^[-\s]*/, '');
        if (!value || value.startsWith('#') || value.startsWith('-')) continue;
        const name = value.split(/[<>=!~;]/)[0].trim().toLowerCase(); if (name) names.add(name.replace(/_/g, '-'));
    }
    const pyproject = path.join(root, 'pyproject.toml');
    if (fs.existsSync(pyproject)) for (const match of readText(pyproject).matchAll(/['\"]([A-Za-z0-9_.-]+)(?:\s*[<>=!~]|['\"])/g)) names.add(match[1].toLowerCase().replace(/_/g, '-'));
    return names;
}

function envKeys(root: string): string[] {
    const keys = new Set<string>(); const files = collectPythonFiles(root, 250);
    const rx = /(?:os\.environ\.get|os\.getenv)\(\s*['\"]([A-Z][A-Z0-9_]+)['\"]/g;
    for (const file of files) { const text = readText(file); let match: RegExpExecArray | null; while ((match = rx.exec(text))) keys.add(match[1]); }
    return [...keys].sort();
}

export function detectFramework(root: string): Framework {
    if (fs.existsSync(path.join(root, 'manage.py'))) return 'django';
    const text = collectPythonFiles(root, 80).map(readText).join('\n');
...
export function analyzeProject(root: string): ProjectReport {
    const files = collectPythonFiles(root); const imports = detectImports(files); const declared = declaredPackages(root);
    const mapped = imports.filter(name => !STDLIB.has(name) && IMPORT_TO_PACKAGE[name]).map(name => IMPORT_TO_PACKAGE[name]);
    const missing = [...new Set(mapped)].filter(pkg => !declared.has(pkg.toLowerCase().replace(/_/g, '-')));
    const framework = detectFramework(root); const hasVenv = fs.existsSync(path.join(root, '.venv')) || fs.existsSync(path.join(root, 'venv'));
    const hasRequirements = fs.existsSync(path.join(root, 'requirements.txt')); const hasPyproject = fs.existsSync(path.join(root, 'pyproject.toml'));
    const hasGitignore = fs.existsSync(path.join(root, '.gitignore')); const keys = envKeys(root); const findings: DoctorFinding[] = [];
    ...
    if (missing.length) findings.push({ severity: 'warning', title: 'Muhtemel eksik bağımlılıklar', detail: missing.join(', '), fix: 'PyOtoBaşlat bunları venv Python ile import testi yapıp gerekirse otomatik kurar.' });
...
export function formatReport(report: ProjectReport): string {
    ...
    const lines = ['# 🐍 PyOtoBaşlat Project Doctor', '', `**Proje:** ${report.root}`, `**Framework:** ${report.framework}`, `**Python dosyası:** ${report.pythonFiles}`, `**Sanal ortam:** ${report.hasVenv ? '✅' : '❌'}`, `**Bağımlılık dosyası:** ${report.hasRequirements || report.hasPyproject ? '✅' : '❌'}`, `**.gitignore:** ${report.hasGitignore ? '✅' : '❌'}`, '', '## Bulgular', ''];
    if (!report.findings.length) lines.push('✅ Kritik veya uyarı niteliğinde bulgu yok.');
    for (const finding of report.findings) { lines.push(`${icon(finding.severity)} **${finding.title}** — ${finding.detail}`); if (finding.fix) lines.push(`   - Öneri: ${finding.fix}`); }
    return lines.join('\n');
}
